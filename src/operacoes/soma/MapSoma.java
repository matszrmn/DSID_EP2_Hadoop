...
public class MapSoma extends Mapper<LongWritable, Text, Text, Text> {
	...
	public void map(LongWritable keyWritable, Text text, Context context) throws IOException, InterruptedException {		
		...
		else if(modoAgrupamento == 2) {
			keyStr = String.valueOf(mes);
			keyStr = String.format("%2s", keyStr);
		}
		else if(modoAgrupamento == 3) {
			keyStr = String.valueOf(diaDoMes);
			keyStr = String.format("%2s", keyStr);
		}
		else keyStr = String.valueOf(dataAtual.get(Calendar.DAY_OF_WEEK));
		key.set(keyStr);
		
		Text value = new Text(linha.substring(indexIni, indexFim).replace(" ", ""));
